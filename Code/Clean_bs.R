...
df_aux <- left_join(df_aux,var_lab_in,by = c("IDNO","Bank"))
df_aux <- df_aux %>% filter(!(month(Date) == 3 & year(Date) >= 1900 & year(Date) <= 1906))
...
# df_aux2 <- left_join(df_aux2,var_lab_in[,c(2,3)],by = c("IDNO")) # CHANG
df_aux2 <- left_join(df_aux2,var_lab_in,by = c("IDNO"))
# df_aux2 <- rename(df_aux2, "Bank" = "Bank")

df_aux2[df_aux2 %in% c("NIL","Nil")] <- NA
df_aux2[,-which(colnames(df_aux2) %in% c("Bank","Date","bank"))] <- lapply(df_aux2[,-which(colnames(df_aux2) %in% c("Bank","Date","bank"))], function(x) {
  as.numeric(as.character(x))
}) # Transform all columns but Bank and Date into numeric
...
# df_all <- left_join(df_all,var_lab_in[,which(colnames(var_lab_in) %in% c("Bank","bank"))], by = "Bank") # Angela's balace sheets already have IDNO so I added IDNO for the other balance sheets aboveas well
df_all <- df_all %>% select(-Bank)


# add manual entries and merge with non-manual

filename <- paste0("./Dataset/balance_sheets/Manual.xlsx")
df_manual <- read_excel(filename, sheet = "manual", skip = 0)
df_manual <-  df_manual %>% select(-real_date)
df_manual$Date <- as.Date(paste(df_manual$year,df_manual$month,last_day(df_manual$month), sep = "-"))
df_manual <- left_join(df_manual,var_lab_in,by = c("bank"))
df_manual <- df_manual %>% select(-Bank)

df_all <- rbind(df_all,df_manual)

...
branches <- left_join(branches,ref_11_cd[,which(colnames(ref_11_cd) %in% c("district","province","population","pop_01","value",
...
# CC <- melt(data = CC, id.vars = c("year"), value.name = "CC" )
...
for (i in unique(df_all$IDNO)){
  ...
  a5 <- df_all %>% filter(year == 1876, IDNO == i) %>%
  ...
  aux <- rowSums(data.frame(df_all$iba[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == i)]),na.rm = T)
  # df_all$IBduefrEW[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == 1)] <- aux*a5$p_frew[1]
  # df_all$IBduefrUK[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == 1)] <- aux*a5$p_fruk[1]
  df_all$iba_row[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == i)] <- aux*a5$p_frewuk[1]
  df_all$ibl_row[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == i)] <- aux*a5$p_toewuk[1]
  df_all$iba[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == i)] <- aux*a5$p_iba[1]
  df_all$ibl[which(df_all$Date < unique(a5$Date[1]) & df_all$IDNO == i)] <- aux*a5$p_ibl[1]
...
q_b_d <- aggregate(q_b[,-which(colnames(q_b) %in% c("district","IDNO","year","province"))], 
                   by =list("district" = q_b$district, "IDNO" = q_b$IDNO, "year" = q_b$year, "province" = q_b$province), 
                   FUN = function(x) extract(x,1))

q_b_dcov <- aggregate(q_b[,-which(colnames(q_b) %in% c("district","year","province"))], 
                   by =list("district" = q_b$district, "year" = q_b$year,"province" = q_b$province), 
                   FUN = function(x) extract(x,1))

# THERE'S AN ERROR HERE. IF ONE BANK HAS BRANCHES IN TWO DISTRICTS WITH THE SAME NAME ONLY ONE IS CHOSEN.
q_b_d_prov <- aggregate(q_b[,-which(colnames(q_b) %in% c("district","IDNO","year"))], 
                   by =list("district" = q_b$district, "IDNO" = q_b$IDNO, "year" = q_b$year), 
                   FUN = function(x) extract(x,1))

q_b_dcov_prov <- aggregate(q_b[,-which(colnames(q_b) %in% c("district","year"))], 
                      by =list("district" = q_b$district, "year" = q_b$year), 
                      FUN = function(x) extract(x,1))

nd_year_prov <-  q_b_dcov %>% group_by(year,province) %>% tally(name = "nd_cov")
nd_year <-  q_b_dcov %>% group_by(year) %>% tally(name = "nd_cov")

pop_year <-  q_b_dcov %>% group_by(year) %>% summarise("pop_year" = sum(population, na.rm = T), "pop_01_year" = sum(pop_01, na.rm = T))
pop_year_prov <-  q_b_dcov %>% group_by(year,province) %>% summarise("pop_year" = sum(population, na.rm = T), "pop_01_year" = sum(pop_01, na.rm = T))

q_b_d_prov <- q_b_d_prov %>% group_by(IDNO,year,province) %>% tally(name = "nd")
...
for (t in unique(q_b_d$year)){
  for (i in unique(q_b_d$IDNO[which(q_b_d$year == t)])){
      q_b_d$nd_noverl[which(q_b_d$year == t & q_b_d$IDNO == i)] <- 
        sum(!(unique(q_b$district[which(q_b$year == t & q_b$IDNO == i)]) %in% unique(q_b$district[which(q_b$year == t & !(q_b$IDNO == i))])))
      q_b_d$nb_noverl[which(q_b_d$year == t & q_b_d$IDNO == i)] <- 
        sum(!(q_b$district[which(q_b$year == t & q_b$IDNO == i)] %in% unique(q_b$district[which(q_b$year == t & !(q_b$IDNO == i))])))
  }
}
  ...
  for (i in unique(q_b_d_prov$IDNO[which(q_b_d_prov$year == t)])){
    for (j in unique(q_b_d_prov$province[which(q_b_d_prov$year == t & q_b_d_prov$IDNO == i)])){
      q_b_d_prov$nd_noverl[which(q_b_d_prov$year == t & q_b_d_prov$IDNO == i & q_b_d_prov$province == j)] <- 
        sum(!(unique(q_b$district[which(q_b$year == t & q_b$IDNO == i & q_b$province == j)]) %in% unique(q_b$district[which(q_b$year == t & !(q_b$IDNO == i) & q_b$province == j)])))
      q_b_d_prov$nb_noverl[which(q_b_d_prov$year == t & q_b_d_prov$IDNO == i & q_b_d_prov$province == j)] <- 
        sum(!(q_b$district[which(q_b$year == t & q_b$IDNO == i & q_b$province == j)] %in% unique(q_b$district[which(q_b$year == t & !(q_b$IDNO == i) & q_b$province == j)])))
    }
  }
}
...
CC <- c("Montreal City","Toronto City","Halifax City & County","St John City & County","Victoria City","Winnipeg","Vancouver City")
q_b_cc <- NULL # comment to drop cc

q_b_cc <- q_b %>%
  mutate(ncc = district %in% CC)
...
q_b_cc <- q_b_cc %>% select(-district) %>% group_by(year,province,IDNO) %>%
  summarise_all(sum)




# for (t in unique(q_b$year)){
#   for (j in unique(q_b$province)){
# 
#   q_b_aux <- q_b %>% filter(year == t, province == j) %>%
#     mutate(ncc = place %in% CC$place[which(CC$year == t & CC$CC == 1)])
#  
#   q_b_cc <-  rbind(q_b_cc,q_b_aux)
# 
#  }
# }
...
# q_b_cc_aux <- q_b_cc %>%
#   group_by(IDNO,year,province,district) %>%
#   summarise(ncc_nb  = sum(ncc), population = extract(population,1))
# 
# q_b_cc_aux2 <- q_b_cc %>%
#   group_by(IDNO,year,province) %>%
#   filter(sum(ncc) == 0) 
# q_b_cc_aux2 <- q_b_cc_aux2 %>%
#   group_by(IDNO,year,province) %>%
#   summarise(ncc = 0)

# q_b_cc <- rbind(q_b_cc_aux,q_b_cc_aux2)
# rm(q_b_cc_aux,q_b_cc_aux2)
...
q_b_agg_prov <- q_b_agg_prov %>% group_by(year,province) %>%
  mutate(n_share = nb/sum(nb,na.rm = T), na.rm = T) %>%
  mutate(n_rel = nb/max(nb,na.rm = T), na.rm = T) %>%
  mutate(cc_rel = ncc_nd/max(ncc_nd,na.rm = T), na.rm = T) %>% # comment to drop cc
  mutate(d_rel = nd/max(nd,na.rm = T), na.rm = T) 
...
q_bs_inc_year <- q_bs_inc %>% filter(month == 3)
q_bs_all_year <- q_bs_all %>% filter(month == 3)
q_bs_inc2_year <- q_bs_inc2 %>% filter(month == 3)
...
qq <- q %>% mutate(group = IDNO %in% c(1))
# q <- q %>% filter(group == 0)

  # q  %>% filter(year %in% c(1882,1889,1894)) %>%
    qq  %>%
  # ggplot(aes(x=d_rel,y=(secs + callc + callosc)/Totalass)) +
      # ggplot(aes(x=d_rel,y=(secs + callc + callosc)/Totalass)) +
  ...
  #  
    ...
    q <- q %>% mutate(group = cc_rel <= 1/5)
      # q  %>% filter(!(IDNO == 21)) %>%
      q_bs_inc_year %>% filter(year <= c(1910)) %>% 
      # ggplot(aes(x=d_cover,y=(iba + Nchqobks)/Totalass)) +
        ggplot(aes(x=ncc_p,y=(iba)/Totalass)) +
        # ggplot(aes(x=d_cover,y=(iba)/Totalass)) +
      geom_point(aes(group = IDNO, color = as.factor(year)), size = 3, alpha = 0.5) +
      # geom_point(size = 3, alpha = 0.5)
      # + xlim(0,5e7)
      geom_label(aes(label = year) ) +
      # geom_label(aes(label = year) ) +
...
q <- q %>% group_by(IDNO,province) %>% mutate(var_nd = nd - Lag(nd),var_nb = nb - Lag(nb), var_ndC =  ndC - Lag(ndC), var_ncc_nb =  ncc_nb - Lag(ncc_nb), var_ncc_nd =  ncc_nd - Lag(ncc_nd))
q <- q %>% group_by(IDNO,province) %>% mutate(var_year = year - Lag(year))
q <- q %>% mutate(var_nb_py = var_nb/var_year,var_nd_py = var_nd/var_year,var_ndC_py = var_ndC/var_year,var_ncc_nb_py = var_ncc_nb/var_year,var_ncc_nd_py = var_ncc_nd/var_year)
...
# q <- q %>% group_by(IDNO) %>% mutate(var_nd = nd - Lag(nd),var_nb = nb - Lag(nb), var_ndC =  ndC - Lag(ndC), var_ncc_nb =  ncc_nb - Lag(ncc_nb), var_ncc_nd =  ncc_nd - Lag(ncc_nd))
...
q_bs_inc_year %>% filter(year %in% c(1869,1870,1876,1882,1889,1894,1900,1905,1910,1920)) %>%
    ggplot() +
    geom_density(aes(ncc_nd/ncc_year,group = year, color = as.factor(year)), size = 1, alpha = 0)
...
q_bs_inc_year <- q_bs_inc_year %>% group_by(IDNO) %>%  mutate(var_nb_pyp = var_nb_py/lag(nb, order_by = Date),
  ...
  q %>% filter(year %in% c(1901:1910)) %>%
    # q_bs_inc_year %>% filter(!(IDNO == 43)) %>%
    # q_bs_inc_year %>% filter(year %in% c(1882,1889,1894,1900:1910)) %>%
      # q_bs_inc_year %>% filter(year %in% c(1882,1889,1894,1900:1910)) %>%
    # ggplot(aes(x = nb_p, y = a - rowa - iba_row - currloans)) +
      # ggplot(aes(x = ncc_cover , y = iba/Totalass ,group = year, color = as.factor(year))) +
      ggplot(aes(x = pop , y =  var_nd_py ,group = year, color = as.factor(year))) +
    geom_point(size = 2, alpha = 1)  +
    stat_smooth(method = "lm", formula = y ~ x , size = 1, se = F) +
    geom_label(aes(label = IDNO) ) +
...
b <- as.data.frame(branches) %>% select(-geometry)
a <- b %>% filter(year %in% c(1901,1910)) %>%  group_by(year,district,province) %>%  tally() 
r <- ref_11_cd %>% select(district,province,pop_01,population)
r$pop_01[which(is.na(r$pop_01))] <- 0
colnames(r) <- c("district","province","1901","1910")
r <- melt(r)
colnames(r) <- c("district","province", "year","pop")
r$year <- as.numeric(as.character(r$year))
z <- left_join(r,a, by = c("district","province","year"))  
z <- z %>% mutate(pc = n/(pop/1e5))  


z %>% filter(year == 1910, pop > 0) %>%
  ggplot(aes(x = pop, y = pc ,group = province, color = as.factor(province))) +
  geom_point(size = 2, alpha = 1)  +
  ylim(0,1000)
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1, se = F)
